...
// Pengaman terakhir: kolom Harga sheet bulanan dipakai formula Fee/Total Fee.
// Teks (mis. rentang "5000-7000") membuat Fee jadi #VALUE! dan Total Fee 0,
// jadi ditolak di sini walaupun UI sudah menyaring.
function pastikanHargaAngka(patch) {
  if (!patch || !("harga" in patch)) return;
  if (norm(patch.harga) === "") return; // boleh dikosongkan
  const c = cekHargaBulanan(patch.harga);
...
// Fee memakai Tarif (kolom Y) bila diisi — dipakai saat guru telat dan
// tarifnya berbeda; kalau kosong, jatuh-balik ke harga katalog.
// Status "Cancel" -> Fee 0 dan kuotanya kembali ke Sisa. Aturannya dipasang di
// RUMUS, bukan hanya disaring di dashboard, supaya siapa pun yang menjumlah
// kolom Fee langsung di spreadsheet mendapat angka yang sama.
// (SUMIFS "<>Cancel" tidak peka huruf besar/kecil, sama seperti LOWER() di Fee.)
const feeFormula = (L, r) =>
  `=IF(LOWER(${L.col.status}${r})="cancel";0;${L.colJml}${r}*IF(Y${r}="";XLOOKUP(K${r};$B:$B;$F:$F;"");Y${r}))`;
const bulanFormula = (r) => `=IF(J${r}="";"";TEXT(J${r};"yyyy-mm"))`;
const sisaFormula = (L, r) =>
  `=G${r}-SUMIFS($${L.colJml}:$${L.colJml};$K:$K;B${r};$${L.col.status}:$${L.col.status};"<>Cancel")`;

// Dipakai juga oleh months.js & skrip migrasi, supaya rumus hanya punya satu sumber.
export const rumusFee = (tab, r) => feeFormula(layoutOf(tab), r);
export const rumusSisa = (tab, r) => sisaFormula(layoutOf(tab), r);

// --- Resolusi tab -----------------------------------------------------------
let RESOLVED_TAB = null;
let _tabAt = 0;

// Sheet bulanan dikenali dari ISINYA (header baris 9), bukan dari namanya.
// Dulu dicocokkan dengan pola nama "…Proyek ASN & Bappenas…", dan begitu tab
// diganti nama jadi "Agustus_Freelance" bulan itu lenyap dari dashboard.
// Menebak dari nama bulan saja juga salah: "Distribusi ASN Juli 5 3" dan
// "Guru_TPA_JULI" memuat nama bulan, dan kolom B/K Distribusi berisi data lain
// yang akan terbaca sebagai katalog & log Juli palsu.
const SIDIK_HEADER = { B: "id project", D: "subtes", G: "kebutuhan", H: "sisa", K: "id project" };
const hdr = (v) => String(v ?? "").replace(/\s+/g, " ").trim().toLowerCase();
const KOLOM_SIDIK = (col) => col.charCodeAt(0) - "B".charCodeAt(0); // posisi dalam B9:K9

let _daftar = { at: 0, list: null, diabaikan: [] };
/** Lupakan cache daftar tab — dipanggil setelah tab dibuat/diganti nama. */
export function lupakanDaftarTab() {
  _daftar = { at: 0, list: null, diabaikan: [] };
}
/** Tab bernama bulan yang sengaja tidak dipakai, beserta alasannya. */
export const tabDiabaikan = () => _daftar.diabaikan;

// Bulan yang DISEMBUNYIKAN dari dashboard (pemilih bulan, analisis, halaman
// guru). Sheet-nya tetap utuh di spreadsheet sebagai arsip pembayaran —
// keputusan 24 Sep 2026: dashboard dimulai dari September, Juni–Agustus
// disimpan, bukan dihapus. Hanya TAMPILAN yang disaring: listMonthTabs tetap
// mengembalikan semua bulan, karena pengaman hapus master & ganti jenis
// harus tetap melihat tautan di bulan tersembunyi.
export const BULAN_DISEMBUNYIKAN = ["Juni", "Juli", "Agustus"];
export const bulanTampil = (list) => list.filter((m) => !BULAN_DISEMBUNYIKAN.includes(m.bulan));

export async function listMonthTabs(ap) {
  const params = ap || (await authParams());
  if (!params) return [];
  const now = Date.now();
  if (_daftar.list && now - _daftar.at < 60000) return _daftar.list;

  const titles = await tabTitles(ID, params);
  const calon = titles.filter((t) => monthIdx(t) !== -1);
  const grids = calon.length ? await batchRead(ID, calon.map((t) => `'${t}'!B9:K9`), params) : [];

  const diabaikan = [];
  const cocok = calon.filter((t, i) => {
    const baris = (grids[i] || [])[0] || [];
    const sel = (c) => hdr(baris[KOLOM_SIDIK(c)]);
    if (Object.entries(SIDIK_HEADER).every(([c, mau]) => sel(c) === mau)) return true;
    // Hanya laporkan yang MIRIP sheet proyek tapi rusak headernya — tab
    // seperti "Distribusi…" memang bukan sheet proyek, tak perlu diributkan.
    if (sel("B") === "id project" || sel("K") === "id project") {
      diabaikan.push({ tab: t, alasan: "header baris 9 tidak cocok dengan format sheet proyek" });
    }
    return false;
  });

  // Satu tab per bulan. Kalau ada dua (mis. "Salinan September_Freelance" yang
  // belum diganti nama), menangkan yang namanya DIAWALI nama bulan.
  const awalBulan = (t) => t.trim().toLowerCase().startsWith(MONTHS[monthIdx(t)].toLowerCase());
  cocok.sort((a, b) => Number(awalBulan(b)) - Number(awalBulan(a)) || titles.indexOf(a) - titles.indexOf(b));
  const perBulan = new Map();
  for (const t of cocok) {
    const i = monthIdx(t);
    if (perBulan.has(i)) diabaikan.push({ tab: t, alasan: `bulan ${MONTHS[i]} sudah diwakili tab "${perBulan.get(i)}"` });
    else perBulan.set(i, t);
  }

  const list = [...perBulan]
    .map(([idx, tab]) => ({ tab, bulan: MONTHS[idx], idx }))
    .sort((a, b) => a.idx - b.idx);
  _daftar = { at: now, list, diabaikan };
  return list;
}
...
export async function resolveTab(ap) {
  ...
  try {
    // Bulan bawaan dipilih dari yang tampil saja; kalau semuanya tersembunyi,
    // jatuh ke semua bulan supaya dashboard tidak kosong.
    const semua = await listMonthTabs(params);
    const list = bulanTampil(semua).length ? bulanTampil(semua) : semua;
    let pick = null;
...
function mapAssignments(rows, L, tarif = []) {
  ...
  rows.forEach((row, i) => {
    ...
    out.push({
      ...
      // Kolom Y & Z: tarif khusus baris ini (mis. guru telat) + alasannya.
      // Kosong = pakai harga katalog.
      tarif: parseNum((tarif[i] || [])[0]),
      ketTarif: norm((tarif[i] || [])[1]),
    });
  });
  return out;
}
...
export async function getBoard(bulan) {
  ...
  const baca = async (tab) => {
    ...
    const [oRows, subIds, gRows, tRows2, tRows] = await batchRead(
      ID,
      [
        q(tab, `B${FIRST_ROW}:H${SCAN_TO}`),
        q(tab, `V${FIRST_ROW}:V${SCAN_TO}`),
        q(tab, `J${FIRST_ROW}:${L.logEnd}${SCAN_TO}`),
        q(tab, `Y${FIRST_ROW}:Z${SCAN_TO}`), // Tarif + Ket. Tarif
        `'${TAB_GURU}'!A1:AD200`,
      ],
      ap
    );
    return {
      ...
      assignments: mapAssignments(gRows || [], L, tRows2 || []),
  ...
  try {
    ...
    hasil.months = bulanTampil(await listMonthTabs(ap));
    hasil.tabDiabaikan = tabDiabaikan();
...
export async function updateAssignment(row, patch, bulan) {
  ...
  // Tarif (Y) & Ket. Tarif (Z) di luar blok log, jadi ditulis terpisah — dan
  // SATU-SATU. Menulisnya sebagai pasangan Y:Z akan menghapus kolom yang tidak
  // ikut dikirim (mis. memperbarui Ket. Tarif saja malah mengosongkan Tarif).
  if ("tarif" in patch) {
    data.push({ range: q(tab, `Y${r}`), values: [[norm(patch.tarif) === "" ? "" : parseNum(patch.tarif)]] });
  }
  if ("ketTarif" in patch) {
    data.push({ range: q(tab, `Z${r}`), values: [[norm(patch.ketTarif)]] });
  }
...
export async function createAssignment(a, bulan) {
  ...
  await batchWrite(ID, [
    { range: q(tab, `J${r}:${L.logEnd}${r}`), values: [greenRowValues(a, r, L)] },
    // Tautan ke Master_Project; tanpa ini baris log tidak terhitung di Analisis.
    { range: q(tab, `W${r}`), values: [[`=IFERROR(XLOOKUP(K${r};$B:$B;$V:$V;"");"")`]] },
    { range: q(tab, `Y${r}:Z${r}`), values: [[norm(a.tarif) === "" ? "" : parseNum(a.tarif), norm(a.ketTarif)]] },
  ]);
  return { row: r, tab };
}

export async function createProject(p, bulan) {
  pastikanHargaAngka(p);
  const ap = await authParams();
  if (!ap) throw new Error("Kredensial Google Sheets belum diset.");
  const tab = await pickTab(bulan, ap);
  const L = layoutOf(tab);
...
export async function deleteAssignment(row, bulan) {
  ...
  const tab = await pickTab(bulan, ap);
  const L = layoutOf(tab);
  const [rows, tar] = await batchRead(
    ID,
    [q(tab, `J${FIRST_ROW}:${L.logEnd}${SCAN_TO}`), q(tab, `Y${FIRST_ROW}:Z${SCAN_TO}`)],
    ap
  );
  const last = lastFilled(rows || []);
  if (r > last) return { deleted: 0 };
  const keep = mapAssignments(rows || [], L, tar || []).filter((a) => a.row > r).sort((x, y) => x.row - y.row);
  const out = [], outTarif = [], outLink = [];
  let cur = r;
  for (const a of keep) {
    const rr = cur++;
    out.push(greenRowValues(a, rr, L));
    // Tarif harus ikut bergeser, kalau tidak akan menempel di baris yang salah.
    outTarif.push([a.tarif || "", a.ketTarif || ""]);
    // W adalah formula turunan yang HARUS menunjuk barisnya sendiri. Kalau tidak
    // ditulis ulang saat baris digeser, ia tetap menunjuk baris lama (meleset).
    outLink.push([`=IFERROR(XLOOKUP(K${rr};$B:$B;$V:$V;"");"")`]);
  }
  out.push(new Array(L.lebar).fill(""));
  outTarif.push(["", ""]);
  outLink.push([""]);
  await batchWrite(ID, [
    { range: q(tab, `J${r}:${L.logEnd}${r + out.length - 1}`), values: out },
    { range: q(tab, `W${r}:W${r + outLink.length - 1}`), values: outLink },
    { range: q(tab, `Y${r}:Z${r + outTarif.length - 1}`), values: outTarif },
  ]);
