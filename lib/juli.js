...
// Nama tab bisa berubah-ubah di spreadsheet (mis. pernah "Juli_Proyek ASN &
// Bappenas" lalu di-rename jadi "..._Freelance", dan muncul tab Agustus).
// Karena itu nama tab TIDAK di-hardcode — dicari dinamis lewat resolveTab().
// TAB_DEFAULT hanya cadangan kalau daftar tab gagal diambil.
export const TAB_DEFAULT = "Juli_Proyek ASN & Bappenas_Freelance";
export const TAB_GURU = "Data guru freelance";
// Urutan bulan untuk memilih tab "bulan terbaru" secara otomatis.
const MONTHS = ["januari","februari","maret","april","mei","juni","juli","agustus","september","oktober","november","desember"];
const monthIndex = (title) => {
  const m = String(title).toLowerCase();
  for (let i = MONTHS.length - 1; i >= 0; i--) if (m.includes(MONTHS[i])) return i;
  return -1;
};
// Kunci ke satu bulan lewat env PROYEK_BULAN (mis. "Juli"); kalau kosong,
// dashboard otomatis mengikuti tab bulan TERBARU yang ada di spreadsheet.
...
// Tab aktif yang sudah di-resolve (di-cache singkat agar tak menembak tiap kali).
let RESOLVED_TAB = null;
let _tabAt = 0;

/**
 * Temukan nama tab "Proyek ASN & Bappenas" untuk bulan target, apa pun
 * suffix/rename-nya. Prioritas: cocok bulan target -> mana saja ASN&Bappenas.
 */
export async function resolveTab(ap) {
  ...
  try {
    ...
    const asnB = titles.filter((t) => /proyek.*asn.*&?.*bappenas/i.test(t));
    let pick = null;
    if (PINNED_MONTH) {
      const re = new RegExp(PINNED_MONTH, "i");
      pick = asnB.find((t) => re.test(t)); // dikunci ke bulan tertentu
    }
    if (!pick) {
      // default: ikuti tab bulan TERBARU (indeks bulan tertinggi)
      pick = asnB.slice().sort((a, b) => monthIndex(b) - monthIndex(a))[0];
    }
    RESOLVED_TAB = pick || asnB[0] || TAB_DEFAULT;
    _tabAt = now;
  } catch (_) {
...
const q = (a1) => `'${RESOLVED_TAB || TAB_DEFAULT}'!${a1}`;
...
export async function getBoard() {
  ...
  const baca = async () => {
    ...
    return {
      source: "live",
      projects: mapProjects(oRows),
      assignments: mapAssignments(gRows),
  ...
  try {
    ...
    if (!hasil.projects.length) return sampleBoard();
    return hasil;
  } catch (e) {
    // Nama tab bisa berubah/dihapus sementara nama lama masih ter-cache. Buang
    // cache, resolve ulang, lalu coba sekali lagi sebelum menyerah ke data contoh.
    try {
      RESOLVED_TAB = null;
      _tabAt = 0;
      await resolveTab(ap);
      const hasil = await baca();
      if (hasil.projects.length) return hasil;
    } catch (e2) {
...
// Paksa sebuah nilai disimpan sebagai TEKS. Tanpa ini, kode seperti "SEP-01"
// atau "MEI-03" diubah Google Sheets menjadi TANGGAL, sehingga SUMIF/XLOOKUP
// tidak pernah cocok dan Sisa/Fee diam-diam salah. Apostrof di depan adalah
// cara resmi Sheets untuk mengunci teks; apostrofnya sendiri tidak ikut tersimpan.
const asText = (v) => {
  const s = norm(v);
  return s === "" ? "" : "'" + s;
};
