...
export async function updateAssignment(row, patch, bulan) {
  ...
  // Tarif (Y) & Ket. Tarif (Z) letaknya di luar blok log, jadi ditulis terpisah.
  if ("tarif" in patch || "ketTarif" in patch) {
    data.push({
      range: q(tab, `Y${r}:Z${r}`),
      values: [[norm(patch.tarif) === "" ? "" : parseNum(patch.tarif), norm(patch.ketTarif)]],
    });
  }
