...
export async function batchWrite(spreadsheetId, data, mode = "USER_ENTERED") {
  ...
  if (!res.ok) {
    ...
    // 403 di sini hampir selalu berarti service account cuma diberi akses
    // "Viewer" pada spreadsheet — membaca jalan, menulis ditolak. Pesan mentah
    // dari Google tidak memberi tahu apa yang harus dilakukan, jadi diganti.
    if (res.status === 403) {
      const creds = getCreds();
      throw new Error(
    ...
    throw new Error(`Gagal menyimpan (HTTP ${res.status}): ${body.replace(/\s+/g, " ").slice(0, 200)}`);
  }
  return res.json();
}
