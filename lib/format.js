...
// Status "Cancel" = pekerjaan batal: tidak dibayar dan kuotanya kembali.
// Aturan yang SAMA dipasang di rumus sheet (Fee -> 0, Sisa tidak terpotong),
// jadi di sini cukup untuk hal yang tak terlihat rumus: kwitansi & hitungan soal.
export const STATUS_BATAL = "Cancel";
export function isBatal(status) {
  return norm(status).toLowerCase() === STATUS_BATAL.toLowerCase();
}
