...
import { SHEET_IDS, authParams, batchRead, batchWrite, tabTitles, getAccessToken, API } from "./gauth";
import { norm, parseNum, cekHargaBulanan } from "./format";
import { serialToISO } from "./juli";
...
/** Semua tab proyek bulanan, urut bulan (terlama -> terbaru). */
export async function listMonths(ap) {
  ...
  return titles
    ...
    .map((t) => ({ tab: t, bulan: monthName(t), idx: monthIndex(t) }))
...
/** Baca semua bulan sekaligus (satu round-trip) — untuk halaman Analisis. */
export async function getAllMonths() {
  const ap = await authParams();
  if (!ap) return { source: "sample", months: [], catalog: [], log: [] };
  const months = await listMonths(ap);
  if (!months.length) return { source: "sample", months: [], catalog: [], log: [] };
  ...
  return { source: "live", months, catalog, log, canWrite: ap.write };
}
...
async function addSheet(title) {
  ...
  const res = await fetch(`${API}/${ID}:batchUpdate`, {
    ...
    body: JSON.stringify({
      requests: [{ addSheet: { properties: { title, gridProperties: { rowCount: 500, columnCount: 25, frozenRowCount: 9 } } } }],
    }),
  });
...
export async function createMonth(bulan, lines = []) {
  ...
  const title = `${nama}_Proyek ASN & Bappenas_Freelance`;
  ...
  const data = [
    ...
    { range: `'${title}'!B9:W9`, values: [HEADER_BARU] },
  ];
  ...
  if (lines.length) {
    ...
    lines.forEach((ln, i) => {
      ...
      sisa.push([`=G${r}-SUMIF($K:$K;B${r};$O:$O)`]);
    });
...
export async function addLines(tab, lines = []) {
  ...
  lines.forEach((ln, i) => {
    ...
    rowsSisa.push([`=G${r}-SUMIF($K:$K;B${r};$O:$O)`]);
  });
