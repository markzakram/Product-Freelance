...
function mapRows(rows) {
  ...
  rows.forEach((row, i) => {
    ...
    out.push({
      ...
      platform: norm(row[4]),
      output: norm(row[5]),
      hargaLengkap: parseNum(row[6]),
      hargaVideo: parseNum(row[7]),
      hargaSoal: parseNum(row[8]),
      hargaLive: parseNum(row[9]),
...
const NUMERIC = new Set(["hargaLengkap", "hargaVideo", "hargaSoal", "hargaLive"]);

const cell = (f, v) => {
  if (v === null || v === undefined || v === "") return "";
  return NUMERIC.has(f) ? parseNum(v) : String(v);
};
